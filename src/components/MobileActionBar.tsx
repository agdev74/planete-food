...
export default function MobileActionBar({ onOpenCart }: MobileActionBarProps) {
  ...
  const phoneNumber = restaurantConfig.whatsappNumber || restaurantConfig.phone.replace(/\D/g, "");
        ...
        <AnimatePresence mode="wait">
          {isMenuPage ? (
                ...
                <div className="relative">
                  ...
                  {totalItems > 0 && (
                    <m.span
                      initial={{ scale: 0 }}
                      animate={{ scale: 1 }}
                      className="absolute -top-2 -right-2 bg-white text-brand-primary text-[10px] font-black w-4 h-4 rounded-full flex items-center justify-center border border-brand-primary"
                    >
                      {totalItems}
                    </m.span>
                  )}
            ...
            >
              ...
              <Link
                href={`/${lang}/menu`}
                className="flex-[1.3] bg-brand-primary text-white py-4 rounded-2xl font-bold flex items-center justify-center gap-2 shadow-lg active:scale-[0.98] transition-all text-[10px] uppercase tracking-widest"
              >
            ...
            >
              ...
              <div className="flex justify-between items-center mb-8">
                <h3 className="font-display font-bold text-xl uppercase italic text-white tracking-tighter">
                  {restaurantConfig.name} <span className="text-brand-primary">{text.contact}</span>
                </h3>
                <button
                  onClick={() => setShowContactMenu(false)}
              ...
              <div className="flex flex-col gap-3">
                {restaurantConfig.phone && (
                  <a href={`tel:${restaurantConfig.phone}`} className="flex items-center justify-between p-5 bg-white/5 rounded-2xl border border-white/5 active:bg-white/10 transition-colors group">
                ...
                {phoneNumber && (
                  <a href={`https://wa.me/${phoneNumber}`} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between p-5 bg-green-500/10 rounded-2xl border border-green-500/10 active:bg-green-500/20 transition-colors group">
                    <div className="flex items-center gap-4">
